...
public class JDBCBaseDao {
	 ...
	 *
	 * <pre>
	 * 保存一个对象,所有不为的null或者""，以及没有标注为{@link com.sz.common.annotation.Transient}的属性
	 * </pre>
	 *
	 * @param t
	 * @return 新插入对象的主键ID
	...
	public <T> int logicDeleteById(Class<T> classz, final String[] list) {
		Assert.notNull(list, "批处理对象集合不能为null");
		Assert.notEmpty(list, "批处理对象集合不能空集合");
		SQLCondition[] cols = new SQLCondition[]{new SQLCondition("deleteState", "=", -1)};
		SQLCondition[] whereCons = new SQLCondition[]{new SQLCondition("id", "in", list)};
	 ...
	 * @return
	 */
	public List<Map<String, Object>> queryForMapList(String sql,Object... objs) {
